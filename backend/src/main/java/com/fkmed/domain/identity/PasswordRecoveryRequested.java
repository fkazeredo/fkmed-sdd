 ...
 * deliver the 30-minute reset link (ADR-0004; SPEC-0004 will centralize delivery). Emitted only
 * when the e-mail matched an account — never for a non-existent e-mail — while the HTTP response
 * stays neutral either way (BR7). The payload carries the account id, delivery e-mail and the raw
 * one-time reset token — never the password.
 *
