...
/**
 * Domain event: an account's password changed (SPEC-0002 §Events, BR10/BR11). Published
 * AFTER_COMMIT and consumed by the infra e-mail listener to deliver the "se não foi você, contate
 * os canais" security notice (ADR-0004; SPEC-0004 will centralize delivery). Serves both the
 * recovery reset and the authenticated self-change, distinguished by {@code flow} (DL-0003). The
 * payload carries only the account id, delivery e-mail and beneficiary — never the password.
 *
