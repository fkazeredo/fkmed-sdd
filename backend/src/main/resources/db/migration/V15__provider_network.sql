-- SPEC-0008 (Provider Network Search), Phase 3: the domain.network module (ADR-0011).
-- Geography registry (DL-0014): municipality (IBGE code PK, name, uf FK -> uf_registry) seeded
-- with the full official IBGE municipality list (source: IBGE localidades API
-- https://servicodados.ibge.gov.br/api/v1/localidades/municipios, fetched 2026-07-05
-- -- 5571 municipalities across the 27 UFs already seeded in uf_registry, V11).
-- Registries service_type (BR5 fixed list, has_specialty_step drives the funnel skip -- only
-- CONSULTORIOS carries the specialty step), specialty (BR6, >= 15 seeded) and seal (BR14,
-- DL-0012 -- parameterizable qualification badges). Provider (+ provider_specialty/provider_seal)
-- referencing municipality + free-text neighborhood (no national neighborhood dataset -- BR3
-- derives offered localities from the active provider base). Provider seed: RJ, >= 6
-- municipalities with active providers, >= 40 active providers overall, >= 10 active Cardiologia
-- providers in Rio de Janeiro/Centro (AC2), a few inactive providers for BR13.
--
-- plan.coverage_uf (DL-0014): nullable UF FK -- ESTADUAL plans store the covered UF, NACIONAL
-- plans leave it null (= every UF). The seeded plan (V1) is ESTADUAL/RJ.


-- Geography registry (DL-0014).
create table municipality (
    ibge_code integer primary key,
    name varchar(100) not null,
    uf varchar(2) not null references uf_registry (code)
);

create index idx_municipality_uf on municipality (uf);

insert into municipality (ibge_code, name, uf) values
...
-- Registries (baseline §0019): service_type, specialty, seal.
create table service_type (
    code varchar(40) primary key,
    name varchar(80) not null,
    has_specialty_step boolean not null default false,
    sort_order integer not null
);

insert into service_type (code, name, has_specialty_step, sort_order) values
    ('CONSULTORIOS', 'Consultórios – Clínicas – Terapias', true, 1),
    ('EXAMES_ESPECIAIS', 'Exames Especiais', false, 2),
    ('HEMODIALISE', 'Hemodiálise', false, 3),
    ('HOSPITAIS_INTERNACAO', 'Hospitais para Internação', false, 4),
    ('LABORATORIOS_EXAMES', 'Laboratórios e Exames', false, 5),
    ('PRONTO_ATENDIMENTO_COMERCIAL', 'Pronto Atendimento – Horário Comercial', false, 6),
    ('PRONTO_SOCORRO_24H', 'Pronto-Socorro 24h (Urgência e Emergência)', false, 7),
    ('TEA', 'TEA', false, 8);
...
insert into seal (code, name, description) values
    ('P', 'Selo P', 'Selo de qualificação do prestador (significado oficial ainda não definido pelo produto — DL-0012).'),
    ('R', 'Selo R', 'Selo de qualificação do prestador (significado oficial ainda não definido pelo produto — DL-0012).'),
    ('ACR_HOSP', 'Acreditação Hospitalar', 'Prestador com acreditação hospitalar reconhecida.'),
    ('ISO9001', 'ISO 9001', 'Prestador certificado ISO 9001 em gestão da qualidade.');

-- Provider directory.
create table provider (
    id uuid primary key,
    name varchar(140) not null,
    service_type_code varchar(40) not null references service_type (code),
    municipality_id integer not null references municipality (ibge_code),
    ...
    constraint chk_provider_cep check (cep is null or cep ~ '^\d{8}$'),
    constraint chk_provider_phone check (phone ~ '^\(\d{2}\) \d{4,5}-\d{4}$')
);

create index idx_provider_municipality on provider (municipality_id);
create index idx_provider_service_type on provider (service_type_code);
create index idx_provider_active on provider (active);
create index idx_provider_neighborhood on provider (municipality_id, neighborhood);

create table provider_specialty (
    provider_id uuid not null references provider (id),
    specialty_code varchar(40) not null references specialty (code),
    primary key (provider_id, specialty_code)
);

create table provider_seal (
    provider_id uuid not null references provider (id),
    seal_code varchar(20) not null references seal (code),
    primary key (provider_id, seal_code)
);

-- Provider seed (RJ) -- fictitious POC reference mass (same convention as V1's canonical seed).
insert into provider (id, name, service_type_code, municipality_id, neighborhood, cep, street, address_number, complement, phone, active) values
    ('b911790b-8fac-42c7-aa1b-3661e3890d7b', 'Clínica Cardiológica Central Ltda', 'CONSULTORIOS', 3304557, 'Centro', '20040002', 'Avenida Rio Branco', '100', null, '(21) 2222-1000', true),
    ('6877be6e-0f72-41a8-a950-e804f6f0d613', 'Instituto do Coração do Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040003', 'Avenida Rio Branco', '110', null, '(21) 2222-1001', true),
    ('8e1939af-4afa-4c18-bb79-42d6866be6dd', 'Consultório Dr. Ricardo Almeida', 'CONSULTORIOS', 3304557, 'Centro', '20040004', 'Avenida Rio Branco', '120', null, '(21) 2222-1002', true),
    ('a9ea06f4-ee99-42d9-91af-75145509302c', 'Cardio Centro RJ', 'CONSULTORIOS', 3304557, 'Centro', '20040005', 'Avenida Rio Branco', '130', null, '(21) 2222-1003', true),
    ('bbfbffee-d3d7-4a68-ad94-33170a198729', 'Clínica CardioVida Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040006', 'Avenida Rio Branco', '140', null, '(21) 2222-1004', true),
    ('f82422d4-966d-49b8-94c5-1606c53ebe10', 'Centro Médico Cardiológico Fluminense', 'CONSULTORIOS', 3304557, 'Centro', '20040007', 'Avenida Rio Branco', '150', null, '(21) 2222-1005', true),
    ('2f3e40b8-f345-4f0b-9d7d-ef1e656fd4cf', 'Consultório Dra. Beatriz Nunes', 'CONSULTORIOS', 3304557, 'Centro', '20040008', 'Avenida Rio Branco', '160', null, '(21) 2222-1006', true),
    ('093b68d1-58a5-4585-b72d-fa85d4005802', 'Clínica do Coração Avenida Rio Branco', 'CONSULTORIOS', 3304557, 'Centro', '20040009', 'Avenida Rio Branco', '170', null, '(21) 2222-1007', true),
    ('1b3a55b1-28a0-49a6-88c8-ce0382bd6823', 'Cardiologia Integrada Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040010', 'Avenida Rio Branco', '180', null, '(21) 2222-1008', true),
    ('eb71ff73-3157-43e8-a058-14ece03b3ebd', 'Instituto Cardiovascular Centro RJ', 'CONSULTORIOS', 3304557, 'Centro', '20040011', 'Avenida Rio Branco', '190', null, '(21) 2222-1009', true),
    ('c4e5e4a5-af28-40df-8f0f-e1b5e420592f', 'Clínica CardioSaúde Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040012', 'Avenida Rio Branco', '200', null, '(21) 2222-1010', true),
    ('88da4a5a-d85d-4261-a40b-ca62146c9d28', 'Clínica Médica Centro RJ', 'CONSULTORIOS', 3304557, 'Centro', '20040020', 'Rua da Assembleia', '10', null, '(21) 2222-2001', true),
    ('4f5365cd-034e-4101-b7d0-a07baf02d484', 'Consultório Dr. Fábio Santos', 'CONSULTORIOS', 3304557, 'Centro', '20040021', 'Rua da Assembleia', '20', null, '(21) 2222-2002', true),
    ('7dafbd7a-5143-412d-9dc5-025a06b153a5', 'Clínica Pediátrica Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040022', 'Rua do Ouvidor', '55', null, '(21) 2222-2003', true),
    ('c6b3d8e1-de05-45ed-918d-163a09239e52', 'Consultório Dra. Camila Rocha', 'CONSULTORIOS', 3304557, 'Centro', '20040023', 'Rua do Ouvidor', '60', null, '(21) 2222-2004', true),
    ('ae58373c-c4f6-4ff6-a37d-6bc7b981e2c1', 'Clínica Ortopédica Central', 'CONSULTORIOS', 3304557, 'Centro', '20040024', 'Rua Uruguaiana', '77', null, '(21) 2222-2005', true),
    ('20f1bc20-a206-4546-8874-ec297cf5ccdb', 'Laboratório Central de Análises Clínicas', 'LABORATORIOS_EXAMES', 3304557, 'Centro', '20040030', 'Avenida Presidente Vargas', '500', null, '(21) 2233-3001', true),
    ('0e6ca7c9-d9ed-49b5-8a5b-25b2856d766c', 'Hospital Municipal do Centro', 'HOSPITAIS_INTERNACAO', 3304557, 'Centro', '20040031', 'Avenida Presidente Vargas', '600', null, '(21) 2233-3002', true),
    ('cc563ba0-a31a-4ec6-900f-3cd5cf6270db', 'Pronto-Socorro 24h Centro RJ', 'PRONTO_SOCORRO_24H', 3304557, 'Centro', '20040032', 'Rua Buenos Aires', '120', null, '(21) 2233-3003', true),
    ('32c0f27b-0cce-4077-a432-3ec6e8ff221e', 'Clínica de Exames Especiais Centro', 'EXAMES_ESPECIAIS', 3304557, 'Centro', '20040033', 'Rua Buenos Aires', '130', null, '(21) 2233-3004', true),
    ('8d3271d6-ca60-4077-9865-8e0177657616', 'Clínica Cardiológica Copacabana', 'CONSULTORIOS', 3304557, 'Copacabana', '22020010', 'Avenida Nossa Senhora de Copacabana', '500', null, '(21) 2255-4001', true),
    ('d7e4b622-e0ba-41c3-84e8-9dc0de9a8109', 'Consultório Dr. André Lima', 'CONSULTORIOS', 3304557, 'Copacabana', '22020011', 'Avenida Nossa Senhora de Copacabana', '520', null, '(21) 2255-4002', true),
    ('3032fc19-fdec-42ba-a29f-f6f7d5e18d04', 'Clínica Dermatológica Copacabana', 'CONSULTORIOS', 3304557, 'Copacabana', '22020012', 'Rua Barata Ribeiro', '300', null, '(21) 2255-4003', true),
    ('223b43c2-850c-40ec-9525-179caaae0ec4', 'Consultório Dra. Renata Vidal', 'CONSULTORIOS', 3304557, 'Copacabana', '22020013', 'Rua Barata Ribeiro', '320', null, '(21) 2255-4004', true),
    ('7b9c241a-1236-476b-a708-9c6a1c0a2fd4', 'Clínica Geriátrica Copacabana', 'CONSULTORIOS', 3304557, 'Copacabana', '22020014', 'Rua Santa Clara', '150', null, '(21) 2255-4005', true),
    ('7353fca4-8805-4dc1-b780-6cd848f460df', 'Clínica Cardiológica Tijuca', 'CONSULTORIOS', 3304557, 'Tijuca', '20510010', 'Rua Conde de Bonfim', '400', null, '(21) 2266-5001', true),
    ('20d3ca86-99b9-4bea-95eb-b63e80b8bd59', 'Consultório Dr. Marcelo Teixeira', 'CONSULTORIOS', 3304557, 'Tijuca', '20510011', 'Rua Conde de Bonfim', '420', null, '(21) 2266-5002', true),
    ('2e6b1e05-6e6f-4764-a350-d599c4f9cc97', 'Clínica Pediátrica Tijuca', 'CONSULTORIOS', 3304557, 'Tijuca', '20510012', 'Rua General Roca', '80', null, '(21) 2266-5003', true),
    ('3b502c43-c07b-4468-b4c5-811c5a8946e3', 'Consultório Dra. Juliana Prado', 'CONSULTORIOS', 3304557, 'Tijuca', '20510013', 'Rua General Roca', '90', null, '(21) 2266-5004', true),
    ('ed38fdad-a722-49b8-8f9e-812f14d9e13f', 'Clínica de Otorrinolaringologia Tijuca', 'CONSULTORIOS', 3304557, 'Tijuca', '20510014', 'Praça Saens Peña', '33', null, '(21) 2266-5005', true),
    ('e58cb0a0-62a3-4776-bda5-ed0c85c190e7', 'Clínica de Hemodiálise Botafogo', 'HEMODIALISE', 3304557, 'Botafogo', '22250010', 'Rua Voluntários da Pátria', '190', null, '(21) 2277-6001', true),
    ('05f0da5d-947b-45e7-9384-5fdfdf67690f', 'Consultório Dr. Caio Ferreira', 'CONSULTORIOS', 3304557, 'Botafogo', '22250011', 'Rua Voluntários da Pátria', '210', null, '(21) 2277-6002', true),
    ('4959d29f-3cb9-4e23-b513-e0845a460015', 'Pronto Atendimento Barra da Tijuca', 'PRONTO_ATENDIMENTO_COMERCIAL', 3304557, 'Barra da Tijuca', '22630010', 'Avenida das Américas', '3000', null, '(21) 2288-7001', true),
    ('bebad1d7-b31d-439f-8e11-93d544872772', 'Clínica TEA Barra da Tijuca', 'TEA', 3304557, 'Barra da Tijuca', '22630011', 'Avenida das Américas', '3200', null, '(21) 2288-7002', true),
    ('8212bdc4-08a8-4e23-94e1-024bf19215fe', 'Clínica Cardiológica Desativada Centro', 'CONSULTORIOS', 3304557, 'Centro', '20040040', 'Rua México', '10', null, '(21) 2222-9001', false),
    ('03988f9f-146d-4112-8a4e-c19d8dba34c5', 'Consultório Dr. Inativo', 'CONSULTORIOS', 3304557, 'Copacabana', '22020020', 'Rua Santa Clara', '10', null, '(21) 2255-9002', false),
    ('0462c57d-9863-4313-8c6a-5cff8127e3be', 'Clínica Cardiológica Cabo Frio', 'CONSULTORIOS', 3300704, 'Centro', '28905010', 'Avenida Assunção', '200', null, '(22) 2645-1001', true),
    ('40c28079-8c68-46ba-9e3c-1da205315269', 'Consultório Dr. Bruno Cardoso', 'CONSULTORIOS', 3300704, 'Centro', '28905011', 'Avenida Assunção', '220', null, '(22) 2645-1002', true),
    ('7f77aab4-07d8-4fa1-a7e9-1643b13d6b08', 'Laboratório Cabo Frio Exames', 'LABORATORIOS_EXAMES', 3300704, 'Centro', '28905012', 'Rua Teixeira e Souza', '80', null, '(22) 2645-1003', true),
    ('4c5f8417-33d8-4503-a44e-1878829c83e5', 'Clínica Cardiológica Niterói', 'CONSULTORIOS', 3303302, 'Icaraí', '24230010', 'Rua Gavião Peixoto', '90', null, '(21) 2622-2001', true),
    ('40118ca3-0538-46a5-900b-ccd29a2aaf19', 'Hospital Regional de Niterói', 'HOSPITAIS_INTERNACAO', 3303302, 'Centro', '24020010', 'Rua Visconde de Uruguai', '400', null, '(21) 2622-2002', true),
    ('b9fe0f30-b22f-4c61-9385-9355b3ac205e', 'Consultório Dra. Patrícia Gomes', 'CONSULTORIOS', 3303302, 'Icaraí', '24230011', 'Rua Gavião Peixoto', '110', null, '(21) 2622-2003', true),
    ('7ec0ec3d-fb6b-4c28-9f62-8bb1af428d46', 'Hospital Desativado Niterói', 'HOSPITAIS_INTERNACAO', 3303302, 'Centro', '24020011', 'Rua Visconde de Uruguai', '420', null, '(21) 2622-2004', false),
    ('0f5821f2-91e4-44a6-a855-5326614c1d67', 'Clínica Médica Três Rios', 'CONSULTORIOS', 3306008, 'Centro', '25802010', 'Avenida Alberto Torres', '250', null, '(24) 2252-3001', true),
    ('4028092c-e54a-4a1d-a89b-149ac6fb60e7', 'Pronto-Socorro 24h Três Rios', 'PRONTO_SOCORRO_24H', 3306008, 'Centro', '25802011', 'Avenida Alberto Torres', '270', null, '(24) 2252-3002', true),
    ('05c6a989-6ec7-4c3d-ab41-b47f26f6adbc', 'Consultório Dr. Eduardo Nascimento', 'CONSULTORIOS', 3304300, 'Centro', '28800010', 'Rua Sebastião de Lacerda', '60', null, '(21) 2637-4001', true),
    ('35347b58-e11a-4f69-b59c-9cc0cdecc87c', 'Consultório Dra. Fernanda Castro', 'CONSULTORIOS', 3304300, 'Centro', '28800011', 'Rua Sebastião de Lacerda', '70', null, '(21) 2637-4002', true),
    ('3474b2d5-975f-4040-9490-444ea5e08c96', 'Clínica Cardiológica Rio das Ostras', 'CONSULTORIOS', 3304524, 'Centro', '28890010', 'Rua Projetada', '15', null, '(22) 2771-5001', true),
    ('2bf09235-398c-42f2-b529-9a9f7dc16d9c', 'Clínica de Exames Especiais Rio das Ostras', 'EXAMES_ESPECIAIS', 3304524, 'Centro', '28890011', 'Rua Projetada', '25', null, '(22) 2771-5002', true);

insert into provider_specialty (provider_id, specialty_code) values
    ('b911790b-8fac-42c7-aa1b-3661e3890d7b', 'CARDIOLOGIA'),
    ('6877be6e-0f72-41a8-a950-e804f6f0d613', 'CARDIOLOGIA'),
    ('8e1939af-4afa-4c18-bb79-42d6866be6dd', 'CARDIOLOGIA'),
    ('a9ea06f4-ee99-42d9-91af-75145509302c', 'CARDIOLOGIA'),
    ('bbfbffee-d3d7-4a68-ad94-33170a198729', 'CARDIOLOGIA'),
    ('f82422d4-966d-49b8-94c5-1606c53ebe10', 'CARDIOLOGIA'),
    ('2f3e40b8-f345-4f0b-9d7d-ef1e656fd4cf', 'CARDIOLOGIA'),
    ('093b68d1-58a5-4585-b72d-fa85d4005802', 'CARDIOLOGIA'),
    ('1b3a55b1-28a0-49a6-88c8-ce0382bd6823', 'CARDIOLOGIA'),
    ('eb71ff73-3157-43e8-a058-14ece03b3ebd', 'CARDIOLOGIA'),
    ('c4e5e4a5-af28-40df-8f0f-e1b5e420592f', 'CARDIOLOGIA'),
    ('88da4a5a-d85d-4261-a40b-ca62146c9d28', 'CLINICA_MEDICA'),
    ('4f5365cd-034e-4101-b7d0-a07baf02d484', 'DERMATOLOGIA'),
    ('7dafbd7a-5143-412d-9dc5-025a06b153a5', 'PEDIATRIA'),
    ('c6b3d8e1-de05-45ed-918d-163a09239e52', 'GINECOLOGIA_OBSTETRICIA'),
    ('ae58373c-c4f6-4ff6-a37d-6bc7b981e2c1', 'ORTOPEDIA_TRAUMATOLOGIA'),
    ('8d3271d6-ca60-4077-9865-8e0177657616', 'CARDIOLOGIA'),
    ('d7e4b622-e0ba-41c3-84e8-9dc0de9a8109', 'ORTOPEDIA_TRAUMATOLOGIA'),
    ('3032fc19-fdec-42ba-a29f-f6f7d5e18d04', 'DERMATOLOGIA'),
    ('223b43c2-850c-40ec-9525-179caaae0ec4', 'GINECOLOGIA_OBSTETRICIA'),
    ('7b9c241a-1236-476b-a708-9c6a1c0a2fd4', 'GERIATRIA'),
    ('7353fca4-8805-4dc1-b780-6cd848f460df', 'CARDIOLOGIA'),
    ('20d3ca86-99b9-4bea-95eb-b63e80b8bd59', 'ENDOCRINOLOGIA'),
    ('2e6b1e05-6e6f-4764-a350-d599c4f9cc97', 'PEDIATRIA'),
    ('3b502c43-c07b-4468-b4c5-811c5a8946e3', 'NEUROLOGIA'),
    ('ed38fdad-a722-49b8-8f9e-812f14d9e13f', 'OTORRINOLARINGOLOGIA'),
    ('05f0da5d-947b-45e7-9384-5fdfdf67690f', 'PSIQUIATRIA'),
    ('8212bdc4-08a8-4e23-94e1-024bf19215fe', 'CARDIOLOGIA'),
    ('03988f9f-146d-4112-8a4e-c19d8dba34c5', 'ORTOPEDIA_TRAUMATOLOGIA'),
    ('0462c57d-9863-4313-8c6a-5cff8127e3be', 'CARDIOLOGIA'),
    ('40c28079-8c68-46ba-9e3c-1da205315269', 'CLINICA_MEDICA'),
    ('4c5f8417-33d8-4503-a44e-1878829c83e5', 'CARDIOLOGIA'),
    ('b9fe0f30-b22f-4c61-9385-9355b3ac205e', 'PEDIATRIA'),
    ('0f5821f2-91e4-44a6-a855-5326614c1d67', 'CLINICA_MEDICA'),
    ('05c6a989-6ec7-4c3d-ab41-b47f26f6adbc', 'CARDIOLOGIA'),
    ('35347b58-e11a-4f69-b59c-9cc0cdecc87c', 'GASTROENTEROLOGIA'),
    ('3474b2d5-975f-4040-9490-444ea5e08c96', 'CARDIOLOGIA');

insert into provider_seal (provider_id, seal_code) values
    ('b911790b-8fac-42c7-aa1b-3661e3890d7b', 'P'),
    ('6877be6e-0f72-41a8-a950-e804f6f0d613', 'R'),
    ('a9ea06f4-ee99-42d9-91af-75145509302c', 'P'),
    ('bbfbffee-d3d7-4a68-ad94-33170a198729', 'R'),
    ('2f3e40b8-f345-4f0b-9d7d-ef1e656fd4cf', 'P'),
    ('093b68d1-58a5-4585-b72d-fa85d4005802', 'R'),
    ('eb71ff73-3157-43e8-a058-14ece03b3ebd', 'P'),
    ('c4e5e4a5-af28-40df-8f0f-e1b5e420592f', 'R'),
    ('88da4a5a-d85d-4261-a40b-ca62146c9d28', 'P'),
    ('7dafbd7a-5143-412d-9dc5-025a06b153a5', 'ISO9001'),
    ('ae58373c-c4f6-4ff6-a37d-6bc7b981e2c1', 'R'),
    ('20f1bc20-a206-4546-8874-ec297cf5ccdb', 'ISO9001'),
    ('0e6ca7c9-d9ed-49b5-8a5b-25b2856d766c', 'ACR_HOSP'),
    ('8d3271d6-ca60-4077-9865-8e0177657616', 'P'),
    ('223b43c2-850c-40ec-9525-179caaae0ec4', 'R'),
    ('20d3ca86-99b9-4bea-95eb-b63e80b8bd59', 'P'),
    ('ed38fdad-a722-49b8-8f9e-812f14d9e13f', 'ISO9001'),
    ('e58cb0a0-62a3-4776-bda5-ed0c85c190e7', 'ACR_HOSP'),
    ('0462c57d-9863-4313-8c6a-5cff8127e3be', 'P'),
    ('7f77aab4-07d8-4fa1-a7e9-1643b13d6b08', 'ISO9001'),
    ('4c5f8417-33d8-4503-a44e-1878829c83e5', 'R'),
    ('40118ca3-0538-46a5-900b-ccd29a2aaf19', 'ACR_HOSP'),
    ('3474b2d5-975f-4040-9490-444ea5e08c96', 'P');

-- Plan coverage (DL-0014): ESTADUAL stores the covered UF; NACIONAL leaves it null (all UFs).
alter table plan add column coverage_uf varchar(2) references uf_registry (code);
update plan set coverage_uf = 'RJ' where coverage = 'ESTADUAL';
