# ADR 0007: Module map revision — `domain.card` (Digital Card)
...
`plan.category` column — DL-0006). Its public facade is `CardService` (`@Service`), consumed only by
...
(`BeneficiaryAccess` placement) · DL-0006 (`planCategory` seed value) · DECISIONS-BASELINE
